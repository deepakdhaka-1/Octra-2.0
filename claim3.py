...
def sign_tx(signing_key: SigningKey, tx_data: Dict) -> str:
    # Remove spaces from JSON string to match exact signing format
...
def call_contract(wallet: Wallet, contract: str, method: str, params: List[str]) -> Optional[str]:
    try:
        ...
        # Special handling for claimToken
        if method == "claimToken":
            print("\n[Preparing claimToken transaction]")
            print(f"From: {wallet.address}")
            print(f"To: {contract}")
            print(f"Nonce: {nonce + 1}")
            
            # Ensure empty params array
            params = []
    ...
    while time.time() - start_time < TIMEOUT:
        try:
            ...
            if response.get('status') == "confirmed":
                ...
                print(f"\nTransaction failed: {response.get('error', 'Unknown error')}")
                return False
        except:
            pass
...
def execute_method(wallets: List[Wallet], contract: ContractInterface, method_idx: int):
    ...
    for i, wallet in enumerate(wallets):
        ...
        balance, nonce = get_balance(wallet)
        print(f"Current balance: {balance:.6f} OCT, Nonce: {nonce}")
        ...
        if i < len(wallets) - 1:
            if get_user_input("\nContinue to next wallet? (y/n): ").lower() != 'y':
                break
        ...
        while True:
            ...
            try:
                ...
                if 0 <= method_idx < len(contract.methods):
                    execute_method(wallets, contract, method_idx)
                else:
